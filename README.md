...
- 📝 **Insertion mode** — by default text is pasted via clipboard + Cmd+V (and the previous clipboard is restored). Switch to **Insert via Accessibility** from the menu (under **Smart → Insert via Accessibility**) to type straight into the focused field without touching the clipboard; it falls back to paste automatically in apps where Accessibility insertion isn't supported.
